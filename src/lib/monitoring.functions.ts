...
export const dispatchAlertServerFn = createServerFn({ method: "POST" })
  ...
  .handler(async (ctx) => {
    ...
    if (!authCheck.authorized) {
      throw new Error(authCheck.reason || "Forbidden: Security alerts can only be generated by authorized security officials.");
    }
...
export const deleteObservationServerFn = createServerFn({ method: "POST" })
  .validator((data: { observationId: string | number; userToken?: string; reason?: string }) => data)
  .handler(async ({ data }) => {
    const { authenticateToken, deleteGroundObservation } = await import("./official-auth.service");
    if (!data.userToken) {
      throw new Error("Forbidden: Only verified government officials, dispatchers, or administrators can delete observations.");
    }
    const profile = await authenticateToken(data.userToken);
    if (!profile) {
      throw new Error("Forbidden: Only verified government officials, dispatchers, or administrators can delete observations.");
    }
    const res = await deleteGroundObservation(profile, String(data.observationId), data.reason);
    if (!res.success) {
      throw new Error(res.error || "Failed to delete observation.");
    }
    return res;
  });
